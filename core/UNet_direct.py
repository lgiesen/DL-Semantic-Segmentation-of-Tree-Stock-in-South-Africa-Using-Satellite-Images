...
input_label_channel = [config.input_shape[-1]]
layer_count = 64
...
input_img = layers.Input(input_shape, name='Input')

c1 = layers.Conv2D(1*layer_count, (3, 3), activation='relu',
...
output = layers.Conv2D(len(input_label_channel), (1, 1),
                       activation='sigmoid', kernel_regularizer=regularizers)(c9)

model = models.Model(inputs=[input_img], outputs=[output])
# model = Model(inputs=[input_img], outputs=[output])
model.compile(optimizer='adam', loss='binary_crossentropy',
              metrics=['accuracy', 'tversky', 'accuracy', 'dice_coef', 'dice_loss', 'specificity', 'sensitivity'])
print(type(model))
model.summary()
